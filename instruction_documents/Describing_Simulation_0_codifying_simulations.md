...
### Example: Temperature Control Module

The workspace now includes a concrete plugin under `workspaces/Describing_Simulation_0/src/simulations/temperature/`. The module exports `createTemperatureControlSystems`, which returns thermostat and drift systems ready for registration through the `inject` command. Both simulation and evaluation players accept module-path injections with payloads shaped as:
...
```
{
  ...
  "type": "inject",
  "payload": {
    "modulePath": "/abs/path/to/src/simulations/temperature/index.ts",
...
`tools/run_integration.sh` exercises this flow end-to-end, capturing baseline, post-injection, and evaluation frames plus a temperature trace under `verifications/<timestamp>/`. Running the script (or the HTTP harness) is the quickest way to validate new systems against the spec before layering additional behavior.
