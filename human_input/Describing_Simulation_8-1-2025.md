...
│   │
│   ├── messaging/
│   │   ├── OutboundMessageBus.ts
│   │   └── OutboundMessageType.ts
│   │
│   ├── routes/
│   │   ├── apiRoutes.ts
│   │   ├── controls.ts
...
This speaks somewhat to the absurdism of the approach. Why do we spend more time producing more text than code that actually runs? The root of re-writability artifacts that translates to interfaces and business logic with some but minimal variation. Our base assumption is implementation tooling (including LLM assistance) and hardware improves, but human operators have relatively static capabilities in understanding code and algorithmic efficiency is fundamentally unchanging.
...
### IV. Outbound Messaging

The messaging feature set will be used by the engine to receive input and transmit signals to the system orchestrating layer. Implementing this now allows us to next build condition evaluation which produces an exit signal to the orchestrator.

#### OutboundMessageBus

The OutboundMessageBus is the single outbound channel from the ECS. It is a concrete implementation that supports exactly one listener, to be the orchestrator.

#### OutboundMessageSystem

The outbound message system is the only system that can write to the outbound message bus. This system is special for being constructed with the bus instance. The message system scans for entities with outbound message components, pushes the payloads, and then flushes these entities and components.

#### OutboundMessageComponent

The outbound message component holds an outbound message type and a payload object, which matches the contract of that type.

#### OutboundMessageType

The outbound message type serves as a definition check between systems that produce messages and the orchestrator which ultimately processes messages. The types to support should be general, but encompassing:

- Data for any extractable information, with a key mapping to the request

- Exit for fulfillment of exit condition

For example, if metrics are to be produced by the engine, the responsible systems create entities with outbound message components of type *data*. The concept of logging falls under *data*, discrete events are reported in framewise samples for the orchestrator to handle. If the exit condition evaluation system has determined the simulation is complete, then one is created with a component of type *exit*.

### V. Condition Evaluation

Support for exit condition detection is required before orchestration of the engine. Now that we have time, we can implement a simple experiment length condition.

#### ExitConditionComponent

Exit condition components contain enough information to determine if the simulation is ending at the cycle the component is being evaluated.

#### ExitConditionSystem

The exit condition system evaluates the exit condition components in the environment and throws an exception.

### VI. Orchestration

In this next checkpoint, we will implement engine orchestration. By the end, we should be able to instantiate a simulation engine that tracks time.

#### ECS

The ECS file contains the initialization of the entity manager and component manager, and the systems evaluation loop.
