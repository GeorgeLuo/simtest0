...
describe("ComponentManager", () => {
  it("attaches a component instance per entity and type", () => {
    ...
    const type = new ComponentType<{ value: number }>("counter");
  ...
  it("removes all components for an entity", () => {
    const { componentManager, entityManager } = makeManagers();
    const entity = entityManager.createEntity();
    const counter = new ComponentType<{ value: number }>("counter");
    const flag = new ComponentType<{ enabled: boolean }>("flag");
  ...
  it("iterates components without allocating intermediate arrays", () => {
    ...
    const visited: Array<{ type: ComponentType<unknown>; data: unknown }> = [];
