...
describe("SystemManager", () => {
  it("initializes systems in order of insertion", () => {
    ...
    expect(first.hooks.init.mock.invocationCallOrder[0]).toBeLessThan(
      second.hooks.init.mock.invocationCallOrder[0],
    );
  });
  ...
  it("supports explicit identifiers and rejects duplicates", () => {
    ...
    expect(() => manager.addSystem(second.system, { id: "alpha" })).toThrow(
