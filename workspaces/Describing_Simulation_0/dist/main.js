...
import { EvaluationPlayer, EVALUATION_FRAME_MESSAGE, } from "./core/evalplayer/EvaluationPlayer.js";
...
export const createServer = (configOverrides = {}) => {
    const config = {
        ...DEFAULT_CONFIG,
        ...configOverrides,
    };
    ...
    const simulationPlayer = createSimulationPlayer(simulationInboundBus, simulationOutboundBus, pluginDirectory);
    const evaluationPlayer = createEvaluationPlayer(evaluationInboundBus, evaluationOutboundBus);
    const router = new Router();
    const createMessageId = createMessageIdFactory();
    const cleanupTasks = [];
    cleanupTasks.push(pipeSimulationFramesToEvaluation(simulationOutboundBus, evaluationInboundBus, createMessageId));
...
function createSimulationPlayer(inboundBus, outboundBus, pluginDirectory) {
...
function createEvaluationPlayer(inboundBus, outboundBus) {
    const componentManager = new ComponentManager();
    const entityManager = new EntityManager(componentManager);
    const systemManager = new SystemManager();
    const inboundRegistry = new InboundHandlerRegistry();
...
function pipeSimulationFramesToEvaluation(simulationOutboundBus, evaluationInboundBus, createMessageId) {
...
function createCodebaseDependencies(rootDirectory, pluginDirectory) {
    return {
        rootDirectory,
        pluginDirectory,
        fs: {
