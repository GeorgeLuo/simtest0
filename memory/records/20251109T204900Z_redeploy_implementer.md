...
## Summary
- Rebuilt the Morphcloud deployment from `SNAPSHOT_ID` using the latest workspace bundle, yielding instance `MORPH_INSTANCE_ID` (public URL `https://simeval-morphvm-p62mulvu.http.cloud.morph.so`, auth token `REDACTED_TOKEN`).
...
## Details
- Script invocation (with `--skip-tests` because the test suite already ran locally): `tools/morphcloud_build_instance.sh --snapshot SNAPSHOT_ID --name sim-eval-prod --service-name simeval --repo-url https://github.com/GeorgeLuo/simtest0.git --repo-branch main --host 0.0.0.0 --port 3000 --metadata purpose=sim-eval --skip-tests`.
